...
int *producer(void *ID) {
    ...
    for (i = 0; i < Max_Num - 1; i++) {
        printf("[%s:%d] buf(full:%d, lock:%d, empty:%d)\n", __FUNCTION__, __LINE__, buf->full.__size[0],
               buf->lock.__size[0], buf->empty.__size[0]);
        if (-1 == sem_wait(&buf->full)) {
            perror("sem_wait didn't return success \n");
            pthread_exit((void *)1);
        }
        printf("[%s:%d]=== wait buf->full  buf(full:%d, lock:%d, empty:%d) ===\n", __FUNCTION__, __LINE__,
               buf->full.__size[0], buf->lock.__size[0], buf->empty.__size[0]);
        ...
        printf("[%s:%d]=== wait buf->lock  buf(full:%d, lock:%d, empty:%d) ===\n", __FUNCTION__, __LINE__,
               buf->full.__size[0], buf->lock.__size[0], buf->empty.__size[0]);
        data = 100 * ThreadID + i;
        buf->buffer[in] = data;
        ...
        printf("[%s:%d]=== post buf->lock  buf(full:%d, lock:%d, empty:%d)===\n", __FUNCTION__, __LINE__,
               buf->full.__size[0], buf->lock.__size[0], buf->empty.__size[0]);
        if (-1 == sem_post(&buf->empty)) {
            perror("sem_wait didn't return success \n");
            pthread_exit((void *)1);
        }
        printf("[%s:%d]=== post buf->empty  buf(full:%d, lock:%d, empty:%d)===\n", __FUNCTION__, __LINE__,
               buf->full.__size[0], buf->lock.__size[0], buf->empty.__size[0]);
    }
    if (-1 == sem_wait(&buf->full)) {
        perror("sem_wait didn't return success \n");
...
int *consumer(void *ID) {
    ...
    do {
        ...
        printf("[%s:%d]=== wait buf->empty  buf(full:%d, lock:%d, empty:%d)===\n", __FUNCTION__, __LINE__,
               buf->full.__size[0], buf->lock.__size[0], buf->empty.__size[0]);
        if (-1 == sem_wait(&buf->lock)) {
            perror("sem_wait didn't return success \n");
            pthread_exit((void *)1);
        ...
        printf("[%s:%d]=== post buf->full  buf(full:%d, lock:%d, empty:%d)===\n", __FUNCTION__, __LINE__,
               buf->full.__size[0], buf->lock.__size[0], buf->empty.__size[0]);
        if (-1 == sem_getvalue(&buf->full, &full_value)) {
            perror("sem_getvalue didn't return success \n");
            pthread_exit((void *)1);
...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < num; i++) pthread_join(con[i], NULL);
    for (i = 0; i < num; i++) pthread_join(pro[i], NULL);
