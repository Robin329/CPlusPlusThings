...
static void *thread_func(void *data) {
    ...
    if (rc) FAIL_AND_EXIT("pthread_getschedparam()", rc);

    rc = pthread_mutex_lock(&c_mutex);
    if (rc) FAIL_AND_EXIT("pthread_mutex_lock()", rc);
    thread_started = 1;
    rc = pthread_cond_signal(&cond);
    if (rc) FAIL_AND_EXIT("pthread_cond_signal()", rc);
    rc = pthread_mutex_unlock(&c_mutex);
    if (rc) FAIL_AND_EXIT("pthread_mutex_unlock()", rc);
...
static int create_thread(int prio, pthread_t *tid) {
    ...
    if (rc != 0) FAIL_AND_EXIT("pthread_attr_init()", rc);

    rc = pthread_attr_setschedpolicy(&attr, SCHED_FIFO);
    if (rc != 0) FAIL_AND_EXIT("pthread_attr_setschedpolicy()", rc);

    rc = pthread_attr_setinheritsched(&attr, PTHREAD_EXPLICIT_SCHED);
    if (rc != 0) FAIL_AND_EXIT("pthread_attr_setinheritsched()", rc);

    sp.sched_priority = prio;
    rc = pthread_attr_setschedparam(&attr, &sp);
    if (rc != 0) FAIL_AND_EXIT("pthread_attr_setschedparam()", rc);

    thread_started = 0;

    rc = pthread_create(tid, &attr, thread_func, NULL);
    if (rc) FAIL_AND_EXIT("pthread_create()", rc);

    rc = pthread_mutex_lock(&c_mutex);
    if (rc) FAIL_AND_EXIT("pthread_mutex_lock()", rc);
    while (!thread_started) {
        rc = pthread_cond_wait(&cond, &c_mutex);
        if (rc) FAIL_AND_EXIT("pthread_cond_wait()", rc);
    }
    rc = pthread_mutex_unlock(&c_mutex);
    if (rc) FAIL_AND_EXIT("pthread_mutex_unlock()", rc);
...
int main(void) {
    ...
    if (rc) FAIL_AND_EXIT("set_affinity_single", errno);

    sp.sched_priority = PRIO_MAIN;
    rc = pthread_setschedparam(pthread_self(), SCHED_FIFO, &sp);
    if (rc) FAIL_AND_EXIT("pthread_setschedparam()", rc);

    rc = pthread_mutex_lock(&mutex);
    if (rc) FAIL_AND_EXIT("pthread_mutex_lock()", rc);

    rc = create_thread(PRIO_LOW, &t3);
    if (rc) FAIL_AND_EXIT("create_thread LOW", rc);

    rc = create_thread(PRIO_MED, &t2);
    if (rc) FAIL_AND_EXIT("create_thread MED", rc);

    rc = create_thread(PRIO_HIGH, &t1);
    if (rc) FAIL_AND_EXIT("create_thread HIGH", rc);

    rc = pthread_mutex_unlock(&mutex);
    if (rc) FAIL_AND_EXIT("pthread_mutex_unlock()", rc);

    rc = pthread_join(t1, &r1);
    if (rc) FAIL_AND_EXIT("pthread_join(t1)", rc);

    rc = pthread_join(t2, &r2);
    if (rc) FAIL_AND_EXIT("pthread_join(t2)", rc);

    rc = pthread_join(t3, &r3);
    if (rc) FAIL_AND_EXIT("pthread_join(t3)", rc);
    ...
    if (status == PTS_PASS) printf("Test PASSED\n");
