...
int main(int argc, char *argv[]) {
    ...
    if ((sigemptyset(&act1.sa_mask) != 0) || (sigemptyset(&act2.sa_mask) != 0)) {
        perror("sigemptyset() did not return success\n");
        return PTS_UNRESOLVED;
    }
    if ((sigaction(SIGABRT, &act1, 0) != 0) || (sigaction(SIGALRM, &act2, 0) != 0)) {
    ...
    if ((timer_create(CLOCK_REALTIME, &ev1, &tid1) != 0) || (timer_create(CLOCK_REALTIME, &ev2, &tid2) != 0)) {
