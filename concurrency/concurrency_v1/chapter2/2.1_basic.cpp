...
public:
    void operator()() const { cout << "ok" << endl; }
};
...
// try catch
// 只能捕获轻量级错误,所以如需确保线程在函数之前结束——查看是否因为线程函数使用了局部变量的引用，
// 以及其他原因——而后再确定一下程序可能会退出的途径，无论正常与否，可以提供一个简洁的机制，来做解决这个问题。

// 一种方式是使用“资源获取即初始化方式”(RAII，Resource Acquisition Is
// Initialization)，并且提供一个类，在析构函数中使用join()，
