            ...
            if (i != prototype.size() - 1) {
                shiftTable.insert(std::make_pair(
                    prototype[i], prototype.size() - i -
                                      1));  // Insert the character as key and
                                            // the size of prototype string -
                                            // index of character - 1 as value
            } else {
                shiftTable.insert(std::make_pair(
                    prototype[i],
                    prototype.size()));  // Insert the character as key and the
                                         // size of prototype string as value
            }
        } else {
            if (i != prototype.size() - 1) {
...
bool horspool(const std::string &text, const std::string &prototype) {
    ...
    while (i < text.size()) {
        ...
        for (int z = static_cast<int>(prototype.size() - 1); z >= 0 && flag;
             z--) {  // Checking if all characters of substring are equal with
                     // all characters of string
            if (text[j] == prototype[z]) {
                k++;
                j--;
            } else {
                flag = false;  // If two characters are not equal set flag to
                               // false and break from loop
            }
        }
        ...
        if (k ==
            ...
            if (shiftTable.find(text[i]) != shiftTable.end()) {
                i += shiftTable[text[i]];  // If shift table contains the
                                           // character then shift index as many
                                           // steps as value
            } else {
                i += prototype.size();  // If character does not exist in shift
                                        // table then shift index as many steps
                                        // as size of prototype string
            }
        }
    }
    return false;
...
static void test() {
    ...
    assert(strings::horspool::horspool("c++_algorithms", "c++_algorithms") ==
           true);
