/**
 * @file xor_cipher.cpp
 * @brief Implementation of [XOR
 * cipher](https://en.wikipedia.org/wiki/XOR_cipher) algorithm.
 *
 * @details
 ...
 * This operation is sometimes called modulus 2 addition (or subtraction, which
 * is identical). With this logic, a string of text can be encrypted by applying
 * the bitwise XOR operator to every character using a given key. To decrypt the
 * output, merely reapplying the XOR function with the key will remove the
 * cipher.
 *
 * ### Algorithm
 * Choose the key for encryption and apply XOR operation to each character of a
 * string. Reapplying XOR operation to each character of encrypted string will
 * give original string back.
 *
...
namespace ciphers {
/** \namespace XOR
 * \brief Functions for [XOR cipher](https://en.wikipedia.org/wiki/XOR_cipher)
 * algorithm.
 */
namespace XOR {
/**
