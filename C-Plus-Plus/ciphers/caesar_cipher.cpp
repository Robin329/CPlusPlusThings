 ...
 * @brief Implementation of [Caesar
 * cipher](https://en.wikipedia.org/wiki/Caesar_cipher) algorithm.
 *
 * @details
 ...
 * mathematically as, \f[ E(x) = (x + n)\;\mbox{mod}\; 26\f] while decryption
 * can be described as, \f[ D(x) = (x - n) \;\mbox{mod}\; 26\f]
 *
 * \note This program implements caesar cipher for only uppercase English
...
std::string encrypt(const std::string &text, const int &shift) {
    ...
    for (char c : text) {             // Going through each character
        int place_value =
            get_value(c);  // Getting value of character (i.e. 0-25)
        place_value =
            (place_value + shift) % 26;  // Applying encryption formula
        char new_char = get_char(
            place_value);  // Getting new character from new value (i.e. A-Z)
...
std::string decrypt(const std::string &text, const int &shift) {
    ...
    for (char c : text) {             // Going through each character
        int place_value =
            get_value(c);  // Getting value of character (i.e. 0-25)
        place_value =
            (place_value - shift) % 26;  // Applying decryption formula
        if (place_value < 0) {  // Handling case where remainder is negative
            place_value = place_value + 26;
        }
        char new_char =
            get_char(place_value);  // Getting original character from decrypted
                                    // value (i.e. A-Z)
