 ...
 * algorithm.
 * @details
 * The Hopcroft–Karp algorithm is an algorithm that takes as input a bipartite
 * graph and produces as output a maximum cardinality matching, it runs in
 * O(E√V) time in worst case.
 *
 * ### Bipartite graph
 ...
 * the graph. Equivalently, a bipartite graph is a graph that does not contain
 * any odd-length cycles.
 *
 * ### Matching and Not-Matching edges
 * Given a matching M, edges that are part of matching are called Matching edges
 * and edges that are not part of M (or connect free nodes) are called
 * Not-Matching edges.
 *
 * ### Maximum cardinality matching
 * Given a bipartite graphs G = ( V = ( X , Y ) , E ) whose partition has the
 * parts X and Y, with E denoting the edges of the graph, the goal is to find a
 * matching with as many edges as possible. Equivalently, a matching that covers
 * as many vertices as possible.
 *
 * ### Augmenting paths
 * Given a matching M, an augmenting path is an alternating path that starts
 * from and ends on free vertices. All single edge paths that start and end with
 * free vertices are augmenting paths.
 *
 *
 * ### Concept
...
class HKGraph {
    ...
    std::vector<std::list<int> > adj;  ///< adj[u] stores adjacents of left side
                                       ///< and 0 is used for dummy vertex

    std::vector<int> pair_u;  ///< value of vertex 'u' ranges from 1 to m
    std::vector<int> pair_v;  ///< value of vertex 'v' ranges from 1 to n
    std::vector<int> dist;  ///< dist represents the distance between vertex 'u'
                            ///< and vertex 'v'
...
/**
 * @brief This function counts the number of augmenting paths between left and
 * right sides of the Bipartite graph
...
int HKGraph::hopcroftKarpAlgorithm() {
    ...
    dist = std::vector<int>(
        m + 1);  // dist[u] stores distance of left side vertices
...
bool HKGraph::bfs() {
    ...
    for (int u = 1; u <= m; u++) {
        ...
        if (pair_u[u] == NIL) {
            ...
            dist[u] = INF;  // set distance as infinite so that this vertex is
                            // considered next time for availibility
        }
    }
    ...
    while (!q.empty()) {
        ...
        if (dist[u] < dist[NIL]) {
            ...
            for (it = adj[u].begin(); it != adj[u].end(); ++it) {
                ...
                // If pair of v is not considered so far i.e. (v, pair_v[v]) is
                // not yet explored edge.
                if (dist[pair_v[v]] == INF) {
                    dist[pair_v[v]] = dist[u] + 1;
                    q.push(
                        pair_v[v]);  // Consider the pair and push it to queue
                }
            }
        }
    }
    ...
    return (
        dist[NIL] !=
        INF);  // If we could come back to NIL using alternating path of
               // distinct vertices then there is an augmenting path available
}

/**
 * @brief This functions checks whether an augmenting path is available exists
 * beginning with free vertex u
...
bool HKGraph::dfs(int u) {
    if (u != NIL) {
        ...
        dist[u] = INF;  // If there is no augmenting path beginning with u then
...
void tests() {
    // Sample test case 1
    int v1a = 3, v1b = 5,
        e1 = 2;            // vertices of left side, right side and edges
    ...
    int expected_res1 =
        0;  // for the above sample data, this is the expected output
    int res1 = g1.hopcroftKarpAlgorithm();

    assert(res1 == expected_res1);  // assert check to ensure that the algorithm
                                    // executed correctly for test 1

    // Sample test case 2
    int v2a = 4, v2b = 4,
        e2 = 6;            // vertices of left side, right side and edges
    ...
    assert(res2 == expected_res2);  // assert check to ensure that the algorithm
                                    // executed correctly for test 2

    // Sample test case 3
    int v3a = 6, v3b = 6,
        e3 = 4;            // vertices of left side, right side and edges
    ...
    assert(res3 == expected_res3);  // assert check to ensure that the algorithm
                                    // executed correctly for test 3
}
