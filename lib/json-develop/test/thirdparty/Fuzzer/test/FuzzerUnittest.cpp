...
TEST(Fuzzer, CrossOver) {
    ...
    for (size_t Len = 1; Len < 8; Len++) {
        ...
        for (const Unit &U : Expected)
            if (U.size() <= Len) ExpectedUnitsWitThisLength.insert(U);
...
TEST(FuzzerMutate, AddWordFromDictionary1) {
    TestAddWordFromDictionary(&MutationDispatcher::Mutate_AddWordFromManualDictionary, 1 << 15);
}
...
void TestAddWordFromDictionaryWithHint(Mutator M, int NumIter) {
    ...
    for (int i = 0; i < NumIter; i++) {
        ...
        if (NewSize >= PosHint + sizeof(W) && !memcmp(W, T + PosHint, sizeof(W))) FoundMask = 1;
    }
...
TEST(FuzzerMutate, ChangeASCIIInteger1) {
    TestChangeASCIIInteger(&MutationDispatcher::Mutate_ChangeASCIIInteger, 1 << 15);
}
...
TEST(FuzzerMutate, ChangeBinaryInteger1) {
    TestChangeBinaryInteger(&MutationDispatcher::Mutate_ChangeBinaryInteger, 1 << 12);
}
...
TEST(Corpus, Distribution) {
    ...
    for (size_t i = 0; i < N; i++) C.AddToCorpus(Unit{static_cast<uint8_t>(i)}, 0);
