...
static void maybe_initialize_extra_stats() {
    ...
    for (size_t idx = 0; idx < num_signals; idx++) SetSigaction(crash_signals[idx], crash_handler);
...
int main(int argc, char **argv) {
    ...
    while (__afl_persistent_loop(N)) {
        ...
        if (n_read > 0) {
            ...
            if (slowest_unit_time_secs < unit_time_secs) slowest_unit_time_secs = unit_time_secs;
