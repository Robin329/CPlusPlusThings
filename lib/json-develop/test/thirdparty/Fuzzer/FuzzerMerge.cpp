...
bool Merger::Parse(std::istream &IS, bool ParseCoverage) {
    ...
    while (std::getline(IS, Line, '\n')) {
        ...
        if (Marker == "STARTED") {
            ...
            if (ParseCoverage) {
                ...
                V.clear();
                while (ISS1 >> std::hex >> N) V.push_back(N);
    ...
    if (LastSeenStartMarker != kInvalidStartMarker) LastFailure = Files[LastSeenStartMarker].Name;
...
size_t Merger::Merge(std::vector<std::string> *NewFiles) {
    ...
    for (size_t i = NumFilesInFirstCorpus; i < Files.size(); i++) {
        ...
        std::set_difference(Cur.begin(), Cur.end(), AllFeatures.begin(), AllFeatures.end(),
    ...
    for (size_t i = NumFilesInFirstCorpus; i < Files.size(); i++) {
        ...
        AllFeatures.insert(Cur.begin(), Cur.end());
        if (AllFeatures.size() > OldSize) NewFiles->push_back(Files[i].Name);
    }
...
void Fuzzer::CrashResistantMergeInternalStep(const std::string &CFPath) {
    ...
    for (size_t i = M.FirstNotProcessedFile; i < M.Files.size(); i++) {
        ...
        // Show stats.
        TotalNumberOfRuns++;
        if (!(TotalNumberOfRuns & (TotalNumberOfRuns - 1))) PrintStats("pulse ");
        // Write the post-run marker and the coverage.
        OF << "DONE " << i;
        for (size_t F : Features) OF << " " << std::hex << F;
        OF << "\n";
    }
}
...
void Fuzzer::CrashResistantMerge(const std::vector<std::string> &Args, const std::vector<std::string> &Corpora) {
    ...
    for (size_t i = 1; i <= AllFiles.size(); i++) {
        ...
        auto ExitCode = ExecuteCommand(BaseCmd + " -merge_control_file=" + CFPath);
        if (!ExitCode) {
