...
#define FUZZER_FLAG_UNSIGNED(Name, Default, Description) \
    {#Name, Description, static_cast<int>(Default), nullptr, nullptr, &Flags.Name},
...
static const size_t kNumFlags = sizeof(FlagDescriptions) / sizeof(FlagDescriptions[0]);
...
static void PrintHelp() {
    ...
    size_t MaxFlagLen = 0;
    for (size_t F = 0; F < kNumFlags; F++) MaxFlagLen = std::max(strlen(FlagDescriptions[F].Name), MaxFlagLen);

    for (size_t F = 0; F < kNumFlags; F++) {
        const auto &D = FlagDescriptions[F];
        if (strstr(D.Description, "internal flag") == D.Description) continue;
        Printf(" %s", D.Name);
        for (size_t i = 0, n = MaxFlagLen - strlen(D.Name); i < n; i++) Printf(" ");
        Printf("\t");
...
static const char *FlagValue(const char *Param, const char *Name) {
    ...
    if (Param[0] == '-' && strstr(Param + 1, Name) == Param + 1 && Param[Len + 1] == '=') return &Param[Len + 2];
    return nullptr;
}
...
static void ParseFlags(const std::vector<std::string> &Args) {
    for (size_t F = 0; F < kNumFlags; F++) {
        if (FlagDescriptions[F].IntFlag) *FlagDescriptions[F].IntFlag = FlagDescriptions[F].Default;
        if (FlagDescriptions[F].UIntFlag)
            *FlagDescriptions[F].UIntFlag = static_cast<unsigned int>(FlagDescriptions[F].Default);
...
static void WorkerThread(const std::string &Cmd, std::atomic<unsigned> *Counter, unsigned NumJobs,
    ...
    while (true) {
        ...
        if (Flags.verbosity) Printf("%s", ToRun.c_str());
...
int MinimizeCrashInput(const std::vector<std::string> &Args) {
    ...
    std::string BaseCmd = CloneArgsWithoutX(Args, "minimize_crash", "exact_artifact_path");
    ...
    while (true) {
        ...
        Cmd += " -minimize_crash_internal_step=1 -exact_artifact_path=" + ArtifactPath;
        Printf("CRASH_MIN: executing: %s\n", Cmd.c_str());
...
int FuzzerDriver(int *argc, char ***argv, UserCallback Callback) {
    ...
    if (Flags.minimize_crash) return MinimizeCrashInput(Args);
    ...
    if (!Inputs->empty() && !Flags.minimize_crash_internal_step) Options.OutputCorpus = (*Inputs)[0];
    Options.ReportSlowUnits = Flags.report_slow_units;
    if (Flags.artifact_prefix) Options.ArtifactPrefix = Flags.artifact_prefix;
    if (Flags.exact_artifact_path) Options.ExactArtifactPath = Flags.exact_artifact_path;
    ...
    // Initialize Seed.
    if (Seed == 0) Seed = (std::chrono::system_clock::now().time_since_epoch().count() << 10) + GetPid();
    ...
    for (auto &U : Dictionary)
        if (U.size() <= Word::GetMaxSize()) MD->AddWordToManualDictionary(Word(U.data(), U.size()));
    ...
    if (DoPlainRun) {
        ...
        for (auto &Path : *Inputs) {
            ...
            Printf("Running: %s\n", Path.c_str());
            for (int Iter = 0; Iter < Runs; Iter++) RunOneTest(F, Path.c_str(), Options.MaxLen);
    ...
    if (Flags.merge) {
        if (Options.MaxLen == 0) F->SetMaxInputLen(kMaxSaneLen);
    ...
    for (auto &Inp : *Inputs) {
        ...
        ReadDirToVectorOfUnits(Inp.c_str(), &InitialCorpus, nullptr, TemporaryMaxLen, /*ExitOnError=*/false);
    }

    if (Options.MaxLen == 0) {
        size_t MaxLen = 0;
        for (auto &U : InitialCorpus) MaxLen = std::max(U.size(), MaxLen);
        F->SetMaxInputLen(std::min(std::max(kMinDefaultLen, MaxLen), kMaxSaneLen));
    }

    if (InitialCorpus.empty()) {
        InitialCorpus.push_back(Unit({'\n'})); // Valid ASCII input.
        if (Options.Verbosity) Printf("INFO: A corpus is not provided, starting from an empty corpus\n");
    }
