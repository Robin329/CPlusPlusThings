    ...
    do {
        ...
        if (FindInfo.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            ...
            if ((FilenameLen == 1 && FindInfo.cFileName[0] == '.') ||
                (FilenameLen == 2 && FindInfo.cFileName[0] == '.' && FindInfo.cFileName[1] == '.'))
                continue;
...
static size_t ParseDrive(const std::string &FileName, const size_t Offset, bool Relative = true) {
...
// Returns number of characters considered if successful.
static size_t ParseServerAndShare(const std::string &FileName, const size_t Offset) {
...
static size_t ParseCustomString(const std::string &Ref, size_t Offset, const char *Patt) {
...
std::string DirName(const std::string &FileName) {
    ...
    size_t DirLen = 0, Res;
    while ((Res = ParseDir(FileName, LocationLen + DirLen))) DirLen += Res;
    ...
    if (DirLen) {
        ...
        if (!FileLen) { // Path ended in separator.
            ...
            while (DirLen && !IsSeparator(FileName[LocationLen + DirLen - 1])) --DirLen;
