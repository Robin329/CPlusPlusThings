...
#define CHECK_EXTERNAL_FUNCTION(fn)                     \
    do {                                                \
        if (!(EF->fn)) MissingExternalApiFunction(#fn); \
    } while (false)
...
void Fuzzer::ResetCounters() {
    if (Options.UseCounters) EF->__sanitizer_update_counter_bitset_and_clear_counters(0);
}
...
bool Fuzzer::RecordMaxCoverage(Fuzzer::Coverage *C) {
    ...
    if (Options.UseIndirCalls && EF->__sanitizer_get_total_unique_caller_callee_pairs) {
        uint64_t NewCallerCalleeCoverage = EF->__sanitizer_get_total_unique_caller_callee_pairs();
...
struct MallocFreeTracer {
    ...
    bool Stop() {
        if (TraceLevel)
            Printf("MallocFreeTracer: STOP %zd %zd (%s)\n", Mallocs.load(), Frees.load(),
                   Mallocs == Frees ? "same" : "DIFFERENT");
...
void MallocHook(const volatile void *ptr, size_t size) {
    ...
    if (int TraceLevel = AllocTracer.TraceLevel) {
        ...
        if (TraceLevel >= 2 && EF) EF->__sanitizer_print_stack_trace();
    }
}
...
void Fuzzer::HandleMalloc(size_t Size) {
    if (!Options.RssLimitMb || (Size >> 20) < (size_t)Options.RssLimitMb) return;
...
static void WarnOnUnsuccessfullMerge(bool DoWarn) {
    ...
    Printf("***\n"
           ...
           "*** You will need to filter out crashes from the corpus, e.g. like "
           "this:\n"
           "***   for f in WITH_CRASHES/*; do ./fuzzer $f && cp $f NO_CRASHES; "
           "done\n"
...
void Fuzzer::DumpCurrentUnit(const char *Prefix) {
    ...
    WriteUnitToFileWithPrefix({CurrentUnitData, CurrentUnitData + UnitSize}, Prefix);
}
...
void Fuzzer::AlarmCallback() {
    ...
    if (!RunningCB) return; // We have not started running units yet.
    size_t Seconds = duration_cast<seconds>(system_clock::now() - UnitStartTime).count();
    if (Seconds == 0) return;
...
void Fuzzer::PrintStats(const char *Where, const char *End, size_t Units) {
    ...
    if (MaxCoverage.BlockCoverage) Printf(" cov: %zd", MaxCoverage.BlockCoverage);
    if (size_t N = MaxCoverage.VPMap.GetNumBitsSinceLastMerge()) Printf(" vp: %zd", N);
    if (size_t N = TPC.GetTotalPCCoverage()) Printf(" cov: %zd", N);
    if (auto TB = MaxCoverage.CounterBitmapBits) Printf(" bits: %zd", TB);
    if (size_t N = Corpus.NumFeatures()) Printf(" ft: %zd", N);
    if (MaxCoverage.CallerCalleeCoverage) Printf(" indir: %zd", MaxCoverage.CallerCalleeCoverage);
    if (!Corpus.empty()) {
...
void Fuzzer::PrintFinalStats() {
    ...
    if (Options.DumpCoverage) TPC.DumpCoverage();
...
void Fuzzer::ShuffleAndMinimize(UnitVector *InitialCorpus) {
    ...
    if (Options.ShuffleAtStartUp) ShuffleCorpus(InitialCorpus);
...
size_t Fuzzer::RunOne(const uint8_t *Data, size_t Size) {
    ...
    if (!TPC.UsingTracePcGuard()) {
        if (TPC.UpdateValueProfileMap(&MaxCoverage.VPMap)) Res = 1;
...
void Fuzzer::ExecuteCallback(const uint8_t *Data, size_t Size) {
    ...
    if (CurrentUnitData && CurrentUnitData != Data) memcpy(CurrentUnitData, Data, Size);
...
void Fuzzer::WriteToOutputCorpus(const Unit &U) {
    ...
    WriteToFile(U, Path);
    if (Options.Verbosity >= 2) Printf("Written to %s\n", Path.c_str());
}
...
UnitVector Fuzzer::FindExtraUnits(const UnitVector &Initial, const UnitVector &Extra) {
...
void Fuzzer::Merge(const std::vector<std::string> &Corpora) {
    ...
    for (auto &U : Res) WriteToOutputCorpus(U);
...
void Fuzzer::Loop() {
    ...
    if (Options.DoCrossOver) MD.SetCorpus(&Corpus);
    while (true) {
        auto Now = system_clock::now();
        if (duration_cast<seconds>(Now - LastCorpusReload).count() >= Options.ReloadIntervalSec) {
