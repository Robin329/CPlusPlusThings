...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    try {
        ...
        try {
            ...
            // step 2.2: round trip with adding size annotations but without adding
            // type annonations to container types
            std::vector<uint8_t> vec3 = json::to_ubjson(j1, true, false);

            // step 2.3: round trip with adding size as well as type annotations to
            // container types
