 ...
 *AuthenticationManager and sets the timeToLive. generate(string tokenId, int
 *currentTime) generates a new token with the given tokenId at the given
 *currentTime in seconds. renew(string tokenId, int currentTime) renews the
 *unexpired token with the given tokenId at the given currentTime in seconds. If
 *there are no unexpired tokens with the given tokenId, the request is ignored,
 *and nothing happens. countUnexpiredTokens(int currentTime) returns the number
 *of unexpired tokens at the given currentTime.
 *
 ...
 *at time 1, so nothing happens. authenticationManager.generate("aaa", 2); //
...
public:
    ...
    void renew(string tokenId, int currentTime) {
        ...
        if (token_map.find(tokenId) == token_map.end() || token_map[tokenId] + ttl <= currentTime) {
            return;
        }
    ...
    int countUnexpiredTokens(int currentTime) { return clean(currentTime - ttl); }
