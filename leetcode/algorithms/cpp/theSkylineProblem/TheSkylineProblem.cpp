 ...
 *guaranteed that 0 ≤ Li, Ri ≤ INT_MAX, 0 , and Ri - Li > 0. You may assume all
 *buildings are perfect rectangles grounded on an absolutely flat surface at
 *height 0.
 *
 ...
 *
 * Therefore, we first obtain both the edges of all the N buildings, and store
 * the 2N edges in a sorted array. Maintain a max-heap of building heights while
 * scanning through the edge array: 1) If the current edge is a left edge, then
 * add the height of its associated building to the max-heap; 2) If the edge is
 * a right one, remove the associated height from the heap.
 *
 * Then we take the top value of the heap (yi) as the maximum height at the
 * current edge position (xi). Now (xi, yi) is a potential key point.
 *
 * If yi is the same as the height of the last key point in the result list, it
 * means that this key point is not a REAL key point, but rather a horizontal
 * continuation of the last point, so it should be discarded;
 *
