...
public:
    ...
    //    `There is an array, all of numbers except one appears twice, and that
    //    one only appears once`
    // It means, we can combin two arrays together, one is [1..n], another one is
    // `nums`. Then, you know, we can use the XOR solve this problem.
