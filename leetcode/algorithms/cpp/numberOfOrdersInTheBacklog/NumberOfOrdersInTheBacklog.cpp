 ...
 *smallest price in the backlog. If that sell order's price is smaller than or
 *equal to the current buy order's price, they will match and be executed, and
 *that sell order will be removed from the backlog. Else, the buy order is added
 *to the backlog. Vice versa, if the order is a sell order, you look at the buy
 *order with the largest price in the backlog. If that buy order's price is
 *larger than or equal to the current sell order's price, they will match and be
 ...
 *sell order of the least price, which is 25 and this sell order is removed from
 *the backlog. Then, there are no more sell orders in the backlog, so the 4th
 *order is added to the backlog. Finally, the backlog has 5 buy orders with
 *price 10, and 1 buy order with price 30. So the total number of orders in the
 *backlog is 6.
 *
 * Example 2:
 *
 ...
 *backlog. Finally, the backlog has (1000000000-3) sell orders with price 7, and
