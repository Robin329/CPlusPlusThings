...
public:
    //
    // It's easy to find out how many 3-length slices in an arithmetic array with
    // N length.
    //
    //   len = 3, then  1 slices, sum(1)
    //   len = 4, then  3 slices, sum(1,2) - TWO 3-length slices + ONE 4-length
    //   slice len = 5, then  6 slices, sum(1,2,3) - THREE 3-length slices + TWO
    //   4-length slices + ONE 5-length slice len = 6, then 10 slices,
    //   sum(1,2,3,4) - ... len = 7, then 15 slices, sum(1,2,3,4,5) - ..
    //
