 ...
 *algorithm? What if nums1's size is small compared to num2's size? Which
 ...
 *
 * 2)If nums1 is significantly smaller than nums2 we can only sort nums1 and
 * then binary search every element of nums2 in nums1 with a total complexity of
 * (MlogN) or if nums2 is already sorted we can search every element of nums1 in
 * nums2 in O(NlogM)
 *
 * 3)Just like 2), we can search for every element in nums2, thus having an
 * online algorithm.
 */
...
public:
    vector<int> intersect(vector<int> &nums1, vector<int> &nums2) {
        sort(nums1.begin(),
             nums1.end()); // we sort both vectors in order to intersect
        sort(nums2.begin(),
             nums2.end());    // them later in O(N + M), where N = nums1.size()
