 ...
 *queries = [1,2,3,4,5] Output: [10,10,9,8,6]
 *
 * Constraints:
 *
 * 	2 <= n <= 2 * 10^4
...
/*
   ...
   We assume all of the nodes are independent, and there is no any pair has
   connection. So, we can count the number of edge for every node.
   ...
   Now, if the `query = 2`, we can have two pointer, one is from left, another
   one from right.
        ...
        So, (degree[left] + degree[right] > q), it means we can have the
   following pairs:

         degree:     1+4   2+4   3+4
           node:    [4,2] [3,2] [1,2]

         ( NB: don't worry about [4,2] & [3,2] are not satsfied `a>b`, they
   could be [2,4] & [2,3] )
   ...
   Now, we can deal with pair which are connected, because they have shared
   edges. So, we need go through all of the pairs they are connect.

   And we need check two condition:

   -  `degree[a] + degree[b] > query` ==> this pair had been counted before!
   -  `degree[a] + degree[b] - shared_edges(a,b) < query` ==> this pair should
   be counted!
