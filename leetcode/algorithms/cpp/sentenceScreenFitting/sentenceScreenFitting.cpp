...
//        std::cout << sentence[t+s];
//
//    std::cout << std::setfill ('-') << std::setw(j == cols ? cols:j+1) << ""
<< std::endl; END
...
int Solution::wordsTyping(vector<string> &sentence, int rows, int cols) {
    ...
    int jump = 0;
    int len[N] = {0};
    FOR(i, l) len[i] = (int)sentence[i].size();
    jump += len[i] END jump += l;
