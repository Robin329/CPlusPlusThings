 ...
 *
 * We start off with a table of n numbers. Let's look at the first number, 2. We
 *know all multiples of 2 must not be primes, so we mark them off as non-primes.
 *Then we look at the next number, 3. Similarly, all multiples of 3 such as 3 ×
 *2 = 6, 3 × 3 = 9, ... must not be primes, so we mark them off as well. Now we
 *look at the next number, 4, which was already marked off. What does this tell
 *you? Should you mark off all multiples of 4 as well?
 *
 ...
 *2 = 10, 5 × 3 = 15, 5 × 4 = 20, 5 × 5 = 25, ... can be marked off. There is a
 *slight optimization here, we do not need to start from 5 × 2 = 10. Where
 *should we start marking off?
 *
 * In fact, we can mark off multiples of 5 starting at 5 × 5 = 25, because 5 × 2
 *= 10 was already marked off by multiple of 2, similarly 5 × 3 = 15 was already
 *marked off by multiple of 3. Therefore, if the current number is p, we can
 *always mark off multiples of p starting at p2, then in increments of p: p2 +
 *p, p2 + 2p, ... Now what should be the terminating loop condition?
 *
 ...
 *have already been marked off. When the loop terminates, all the numbers in the
 *table that are non-marked are prime.
 *
 ...
 *you can read more about its algorithm complexity on
