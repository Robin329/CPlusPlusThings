...
double findMedianSortedArrayHelper(int A[], int m, int B[], int n, int lowA, int highA, int lowB,
    ...
    if (num == (m + n) / 2) {
        ...
        // You can see, the `pos` is the place A[mid] can be inserted, so return
        // A[mid]
        if ((m + n) % 2 == 1) {
            return A[mid];
        }
        ...
        // Because the `mid` is the second middle number, we need to find the first
        // middle number Be careful about the edge case
        if (mid > 0 && pos > 0) {
