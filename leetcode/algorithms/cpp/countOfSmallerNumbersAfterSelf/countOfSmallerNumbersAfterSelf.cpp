...
public:
    ...
    int search(int t) {
        ...
        for (int k = step; k >= 0; k--)
            if (i + (1 << k) < n && sorted[i + (1 << k)] <= t) i += (1 << k);
        return i;
    }
    int compute(int t) {
        int s = 0;
        for (int i = t; i > 0; i -= zeros(i)) s += fenwick[i];
        return s;
    }
    void add(int t) {
        for (int i = t; i <= n; i += zeros(i)) fenwick[i]++;
    }
...
public:
    ...
    void insert(const int value, int &numLessThan) { insert(root, value, numLessThan); }
