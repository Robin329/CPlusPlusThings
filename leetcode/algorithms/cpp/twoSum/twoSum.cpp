...
public:
     ...
     *   So, we can use a hashmap to reduce the searching time complexity from
     * O(n) to O(1) (the map's `key` is the number, the `value` is the position)
     *
    ...
    //     so, when we checking the next num[i], if we found it is exisited in the
    //     map. which means we found the second one.
    //
