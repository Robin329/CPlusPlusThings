...
/*
   ...
   - if (start == end) dp[statr, end] = 1, it means every char can be
  palindromic
  ...
  calculating from the bottom to up. (Note: only care about the top-right
  trangle)
  ...
  So, we can have the following formular:
       ...
       s[start] != s[end]  ==> dp[start, end] = max (dp[start+1, end], dp[start,
  end-1]); s[start] == s[end]  ==> dp[start, end] = dp[start+1, end-1] + 2;
