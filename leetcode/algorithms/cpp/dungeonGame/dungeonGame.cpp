 ...
 *in a 2D grid. Our valiant knight (K) was initially positioned in the top-left
 ...
 *initial health of the knight must be at least 7 if he follows the optimal path
 ...
 * even the first room the knight enters and the bottom-right room where the
...
public:
    int calculateMinimumHP(vector<vector<int>> &dungeon) {
        ...
        // dp[r][c] = min( max( dp[r+1][c] - dungeon[r][c], 1), max( dp[r][c+1] -
        // dungeon[r][c], 1) );
        int dp[row][col];
