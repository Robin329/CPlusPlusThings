 ...
 *stream to a separate container. Remove the smallest k elements and the largest
 *k elements from the container. Calculate the average value for the rest of the
 *elements rounded down to the nearest integer.
 *
 * Implement the MKAverage class:
 *
 * 	MKAverage(int m, int k) Initializes the MKAverage object with an empty
 *stream and the two integers m and k. void addElement(int num) Inserts a new
 *element num into the stream. int calculateMKAverage() Calculates and returns
 ...
 *exist. obj.addElement(10);       // current elements are [3,1,10]
...
public:
    ...
    void addElement(int num) {
        ...
        if (pos > m) {
            ...
            sum -= n;
            del(n);
            // cout << "DEL: n=" << n << ", sum=" << sum << ", maxmin=" << maxmin <<
            // endl; print(left); print(mid);print(right);
        }
        ...
        ins(num);
        // cout << "INS: n=" << num << ", sum=" << sum << ", maxmin=" << maxmin <<
        // endl; print(left); print(mid);print(right);
    }

    int calculateMKAverage() {
        if (pos < m) return -1;
        // cout << "CAL: sum=" << sum << ", maxmin=" << maxmin  << ", delta=" << sum
        // - maxmin<< endl;
...
public:
    ...
    void addElement(int num) {
        ...
        if (pos > m) {
            ...
            if (n <= sort[k - 1])
                maxmin += (sort[k] - n);
            else if (n >= sort[len - k])
                maxmin += (sort[len - k - 1] - n);
        ...
        if (sort.size() < 2 * k) {
            ...
            if (num <= sort[k - 1])
                maxmin += (num - sort[k - 1]);
            else if (num >= sort[len - k])
                maxmin += (num - sort[len - k]);
        }
