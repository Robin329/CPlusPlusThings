 ...
 *  2) when reach the top, the reflection would go down, when reach the bottom
 * the reflection would go up.
 *
 *  So, we can image if there have two walls, left one and right one, then the
 * reflection can go up instanstly,
 *
 ...
 *
 *  So, we need find the least common multiple of `p` and `q`, then we can have
 * the answer.
 */
...
private:
    ...
    int leastCommonMultiple(int a, int b) { return a * b / greatestCommonDivisor(a, b); }
