 ...
 *even; OR, for i <= k < j, A[k] > A[k+1] when k is even, and A[k] < A[k+1] when
 *k is odd.
 *
...
public:
    int maxTurbulenceSize_01(vector<int> &A) {
        ...
        enum Status { up, down, none } s = none;
    ...
    // The previous solution is quite straight forward, but the code is a bit
    // complcated the following solution tries to use another way to make the code
    // simpler.
    //
    ...
    //
    //   - UP means the previous status goes up. and the previous length of the
    //   zigzog pattern.
    //   - DOWN is same.
    //
    // So,
    //
    //   - if the previous is UP, then the previous DWON must be 1, and vice
    //   versa.
    //
