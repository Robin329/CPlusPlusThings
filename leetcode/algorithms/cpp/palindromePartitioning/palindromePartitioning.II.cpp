 ...
 *
 *  f(0) = 0; // minimum cut of str[0:0]="l", which is a palindrome, so not cut
 * is needed. f(1) = 1; // str[0:1]="le" How to get 1? f(1) might be:  (1)
 * f(0)+1=1, the minimum cut before plus the current char. (2)   0, if str[0:1]
 * is a palindrome  (here "le" is not ) f(2) = 1; // str[0:2] = "lee" How to get
 * 2? f(2) might be:   (1)  f(1) + 1=2 (2)  0, if str[0:2] is a palindrome (here
 * "lee" is not) (3)  f(0) + 1,  if str[1:2] is a palindrome, yes! f(3) = 2; //
 * str[0:3] = "leet" How to get 2? f(3) might be:   (1)  f(2) + 1=2 (2)  0, if
 * str[0:3] is a palindrome (here "leet" is not) (3)  f(0) + 1,  if str[1:3] is
 * a palindrome (here "eet" is not) (4)  f(1) + 1,  if str[2:e] is a palindrome
 * (here "et" is not) OK, output f(3) =2 as the result.
 *
 ...
 *
 *  The above algorithm works well for the smaller test cases, however for the
 * big cases, it still cannot pass. Why? The way we test the palindrome is
 * time-consuming.
 *
 *  Also using the similar DP idea, we can construct the look-up table before
 * the main part above, so that the palindrome testing becomes the looking up
 * operation. The way we construct the table is also the idea of DP.
 *
 *  e.g.   mp[i][j]=true if str[i:j] is palindrome.
 *         mp[i][i]=true;
 *         mp[i][j] = true if str[i]==str[j] and (mp[i+1][j-1]==true or j-i<2 )
 * j-i<2 ensures the array boundary.
 */
...
int minCut_DP(string &s) {
    ...
    //  3) matrix[i][j] = matrix[i+1][j-1];   if s[i]==s[j]     -- more than two
    //  chars
    //
