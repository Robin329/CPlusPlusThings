 ...
 *by a 2D integer array tasks, where tasks[i] = [enqueueTimei, processingTimei]
 *means that the i^​​​​​​th​​​​ task will be available to
        ...
        while (i < tasks.size()) {
            ...
            while (!q.empty()) {
                ...
                for (; i < tasks.size() && tasks[i][0] <= time; i++) {
                    // cout << "ENQUEUE: " <<  tasks[i][2] << ":[" << tasks[i][0] <<","<<
                    // tasks[i][1] << "]"<< endl;
                    q.push(tasks[i]);
                }
                // print(q);
