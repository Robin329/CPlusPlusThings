 ...
 *cow, and your function should return "1A1B". You may assume that the secret
 *number and your friend's guess only contain digits, and their lengths are
 *always equal.
 *
 * Credits:
...
public:
    ...
    string getHint01(string secret, string guess) {
        ...
        for (int i = 0; i < secret.size(); i++) appears_in_secret[secret[i] - '0']++;
        for (int i = 0; i < guess.size(); i++) {
            if (secret[i] == guess[i]) bulls[secret[i] - '0']++;
