...
static void sig_handler(int signal) {
    if (signal != SIGALRM) {
        fprintf(stderr,
                "sig_handlder(): unexpected signal caught"
                "[%d]\n",
                signal);
        exit(-1);
    } else
...
int main(int argc, char **argv) {
    ...
    while ((c = getopt(argc, argv, "ahps:x:")) != EOF) {
      printf("c:%d\n", c);
      switch (c) {
        case 'a':
          map_anon = TRUE;
    ...
    do {
        ...
        memptr = mmap(0, (fsize * MB), PROT_READ | PROT_WRITE, map_flags, fd, 0);
        ...
        if (msync(memptr, ((fsize * MB) / sizeof(char)), MS_SYNC | MS_INVALIDATE) == -1) {
            perror("main(): msync()");
            exit(-1);
        }
