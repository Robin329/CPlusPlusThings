...
static void sig_handler(int signal,                 /* signal number, set to handle SIGALRM       */
                        int code, ucontext_t *ut) { /* contains pointer to sigcontext structure   */
#ifdef __i386__
    ...
    switch (except) {
        case 10:
            ...
            break;
        case 11:
            fprintf(stderr, "Exception - segment not present, exception #[%ld]\n", except);
            break;
        case 12:
            fprintf(stderr, "Exception - stack segment not present, exception #[%ld]\n", except);
            break;
        case 13:
            fprintf(stderr, "Exception - general protection, exception #[%ld]\n", except);
            break;
        case 14:
            ...
            break;
        default:
            fprintf(stderr, "Exception type not handled... unknown exception #[%ld]\n", except);
            break;
    }
    ...
    if (ret) {
        if (scp->edi == (int)map_address) {
            ...
            fprintf(stderr,
                    ...
                    (unsigned long)scp->edi, (unsigned long)scp->esi, map_address);
...
/*									      */
/******************************************************************************/
static void usage(char *progname) { /* name of this program                       */
    fprintf(stderr,
            "Usage: %s -h -l -x\n"
...
/*									      */
/******************************************************************************/
void *shmat_shmdt(void *args) { /* arguments to the thread X function.          */
    ...
    while (shm_ndx++ < (int)locargs[0]) {
        ...
        if ((shmid = shmget(shmkey, fsize, IPC_CREAT | 0666)) == -1) {
            ...
            fprintf(stdout, "%s[%#lx]: shmget(): success, got segment of size %d\n", STR_SHMAT,
                    pthread_self(), fsize);
        }

        if ((map_address = shmat(shmid, NULL, 0)) == (void *)-1) {
        ...
        fprintf(stdout, "%s[%#lx]: Num iter: [%d] Total Num Iter: [%d]\n", STR_SHMAT,
                pthread_self(), shm_ndx, (int)locargs[0]);
        usleep(0);
        sched_yield();
...
void *write_to_mem(void *args) {
    ...
    while (write_ndx++ < (int)locargs[0]) {
        ...
        while (!done_shmat) usleep(0);

        if (sigsetjmp(jmpbuf, 1) == 1) {
            fprintf(stdout, "page fault ocurred due a write after an shmdt from [%p]\n",
                    map_address);
        }

        fprintf(stdout, "%s[%#lx]: write_to_mem(): memory address: [%p]\n", STR_WRITER,
                pthread_self(), map_address);
...
void *read_from_mem(void *args) {
    ...
    while (read_ndx++ < (int)locargs[0]) {
        ...
        fprintf(stdout, "%s[%#lx]: read_from_mem():  memory address: [%p]\n", STR_READER,
                pthread_self(), map_address);
        if (sigsetjmp(jmpbuf, 1) == 1) {
            fprintf(stdout, "page fault ocurred due a read after an shmdt from %p\n", map_address);
        }

        fprintf(stdout, "%s[%#lx]: read_mem(): content of memory: %s\n", STR_READER, pthread_self(),
                (char *)map_address);
...
int main(int argc,           /* number of input parameters.                        */
    ...
    } sig_info[] = {{SIGHUP, "SIGHUP"},   {SIGINT, "SIGINT"},   {SIGQUIT, "SIGQUIT"},
                    {SIGABRT, "SIGABRT"}, {SIGBUS, "SIGBUS"},   {SIGSEGV, "SIGSEGV"},
                    {SIGALRM, "SIGALRM"}, {SIGUSR1, "SIGUSR1"}, {SIGUSR2, "SIGUSR2"},
                    {-1, "ENDSIG"}};
    ...
    while ((c = getopt(argc, argv, "h:l:x:")) != -1) {
        switch (c) {
            ...
            case 'l': /* number of times to loop in the thread function     */
                if ((num_iter = atoi(optarg)) == 0) num_iter = 1000;
                break;
            case 'x': /* time in hrs to run this test.                      */
                if ((exec_time = atof(optarg)) == 0) exec_time = 24;
                break;
            default:
                usage(argv[0]);
                break;
    ...
    for (;;) {
        ...
        for (thrd_ndx = 0; thrd_ndx < 3; thrd_ndx++) {
            ...
            if (status == (void *)-1) {
                fprintf(stderr, "thread [%#lx] - process exited with errors %ld\n", thid[thrd_ndx],
                        (long)status);
                exit(-1);
            }
        }
