...
#define SHEGET_PARA_SIZE                            (2048)
#define SHEGET_PARA_SHMFLAG_FILE_AUTHORRITY         (666)
#define SHEAT_PARA_SHMFLAG_WR_MODE                  (0)
#define RUN_TIME                                    (30u)
#define DELAY_TIME_LEVELS                           (5.0)

#define TIMESTAMP()                                 do {memset(&cur_time, 0x00, sizeof(cur_time));   \
                                                            time(&cur_time);                          \
                                                            printf("%s", ctime(&cur_time));} while(0)
...
int main()
{
	...
	/**
     * @description:系统建立IPC通讯 （消息队列、信号量和共享内存） 时必须指定一个ID值。通常情况下，该id值通过ftok函数得到
     * @param {char *} fname fname就是你指定的文件名（已经存在的文件名），一般使用当前目录，如：key = ftok(".", 1); 这样就是将fname设为当前目录
     * @param {int} id id是子序号。虽然是int类型，但是只使用8bits(1-255)
     * @return {*}
     *key_t ftok( const char * fname, int id )
    */
    // 1.申请共享内存的key值
	key = ftok(".", FTOK_PARA_ID_VALUE);

    /**
     * @description:得到一个共享内存标识符或创建一个共享内存对象并返回共享内存标识符
     * @param {key_t} key 0(IPC_PRIVATE)：会建立新共享内存对象 | 大于0的32位整数：视参数shmflg来确定操作。通常要求此值来源于ftok返回的IPC键值
     * @param {size_t} size 大于0的整数：新建的共享内存大小，以字节为单位 | 0：只获取共享内存时指定为0
     * @param {int} shmflg 0：取共享内存标识符，若不存在则函数会报错 | IPC_CREAT：当shmflg&IPC_CREAT为真时，如果内核中不存在键值与key相等的共享内  存，则新建一个共享内存；如果存在这样的共享内存，返回此共享内存的标识符 | IPC_CREAT|IPC_EXCL：如果内核中不存在键值 与key相等的共享内存，则新建一个共享内存；如果存在这样的共享内存则报错
     * @return {*} 成功：返回共享内存的标识符 |出错：-1，错误原因存于errno中
     * @note:在Linux环境中，对开始申请的共享内存空间进行了初始化，初始值为0x00，对于参数shmflg一般还会或上一个文件权限
     * int shmget(key_t key, size_t size, int shmflg)
     */
	//2. 申请共享内存ID
	shmid = shmget(key, SHEGET_PARA_SIZE, IPC_CREAT | SHEGET_PARA_SHMFLAG_FILE_AUTHORRITY);

    /**
     * @description:连接共享内存标识符为shmid的共享内存，连接成功后把共享内存区对象映射到调用进程的地址空间，随后可像本地空间一样访问
     * @param {int} shmid 共享内存标识符
     * @param {void} *shmaddr 指定共享内存出现在进程内存地址的什么位置，直接指定为NULL让内核自己决定一个合适的地址位置
	...
	char *p_shm = (char *)shmat(shmid, NULL , SHEAT_PARA_SHMFLAG_WR_MODE);
    ...
    if (p_shm) {
         ...
         * @param {sem_t} *sem
         * @param {int} pshared 指明信号量是由进程内线程共享，还是由进程之间共享。如果值为0，那么信号量将被进程内的线程共享，并且应该放置在这个进程的所有线程都可见的地址上(如全局变量，或者堆上动态分配的变量)
        ...
        int W_creat_ret = pthread_create(&W_pthread_id, &sham_read_pthread_attribute, sham_write, (void *)p_shm);
        if (W_creat_ret != 0) {
        ...
        pthread_attr_init(&sham_read_pthread_attribute);
        pthread_attr_setdetachstate(&sham_read_pthread_attribute, PTHREAD_CREATE_JOINABLE);
        int R_creat_ret = pthread_create(&R_pthread_id, &sham_read_pthread_attribute, sham_read, (void *)p_shm);
        if (R_creat_ret != 0) {
            printf("W:pthread_create is failed ##########\n");
            return -1;
        }
