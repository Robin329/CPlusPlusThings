...
void yuv2yuva(int width, int height, const void* y, const void* u, const void* v, int strideY, int strideU, int strideV,
              void* output, int strideOutput) {
    ...
    for (int h = 0; h < halfHeight; ++h) {
        ...
        unsigned char* output1 = output0 + strideOutput;
        output = output1 + strideOutput;
#if HAVE_NEON
        int halfWidth8 = halfWidth / 8;
        for (int w = 0; w < halfWidth8; ++w) {
            uint8x16_t y00 = vld1q_u8(y0);
            y0 += 16;
            uint8x16_t y10 = vld1q_u8(y1);
            y1 += 16;
            ...
            if (interleaved) {
                ...
                int8x8_t u0000 = vld1_u8(u0);
                u0 += 8;
                int8x8_t v0000 = vld1_u8(v0);
                v0 += 8;
        ...
        for (int w = 0; w < halfWidth16; ++w) {
            __m256i y00 = _mm256_loadu_si256((__m256i*)y0);
            y0 += 32;
            __m256i y10 = _mm256_loadu_si256((__m256i*)y1);
            y1 += 32;
            ...
            _mm256_storeu_si256((__m256i*)output1 + 0, b[0]);
            output0 += 16 * 8;
            _mm256_storeu_si256((__m256i*)output1 + 1, b[1]);
            _mm256_storeu_si256((__m256i*)output1 + 2, b[2]);
            _mm256_storeu_si256((__m256i*)output1 + 3, b[3]);
            output1 += 16 * 8;
        }
        continue;
#elif HAVE_SSE2
        int halfWidth8 = halfWidth / 8;
        for (int w = 0; w < halfWidth8; ++w) {
            __m128i y00 = _mm_loadu_si128((__m128i*)y0);
            y0 += 16;
            __m128i y10 = _mm_loadu_si128((__m128i*)y1);
            y1 += 16;
            ...
            if (interleaved) {
                ...
                u00 = _mm_loadl_epi64((__m128i*)u0);
                u0 += 8;
                v00 = _mm_loadl_epi64((__m128i*)v0);
                v0 += 8;
            }
...
#define yuv2yuva_select(interleaved, firstU, iY, iU, iV, iA) yuv2yuva<interleaved, firstU, iY, iU, iV, iA>
#endif
//------------------------------------------------------------------------------
#ifndef yuv2yuva
//------------------------------------------------------------------------------
...
#define yuv2yuva_select(interleaved, firstU, iY, iU, iV, iA) \
    neon() ? yuv2yuva_neon<interleaved, firstU, iY, iU, iV, iA> : yuv2yuva<interleaved, firstU, iY, iU, iV, iA>
#endif
//------------------------------------------------------------------------------
...
#undef yuv2yuva_select
#define yuv2yuva_select(interleaved, firstU, iY, iU, iV, iA) \
    sse2() ? yuv2yuva_sse2<interleaved, firstU, iY, iU, iV, iA> : yuv2yuva<interleaved, firstU, iY, iU, iV, iAe>
#endif
//------------------------------------------------------------------------------
#if defined(_M_IX86) || defined(_M_AMD64) || defined(__i386__) || defined(__amd64__)
#define HAVE_AVX2 1
