...
#ifndef CPLUSPLUSTHINGS_LOOP_H
#define CPLUSPLUSTHINGS_LOOP_H
#ifndef A_LOOP_H
#define A_LOOP_H
...
public:
     ...
     * @return 创建成功的AMessage
     */
    static std::shared_ptr<AMessage> create(uint32_t what,
     ...
     * @return
     * OK,发送成功；NOT_FOUND，目标handler所在的looper已经停止或未设置;NO_MEM,没有足够内存创建AReplyToken
     */
     ...
     * @return
     * true，发送线程正在阻塞等待（即调用了postAndAwaitResponse）；false，发送线程没有在等待回复
     */
