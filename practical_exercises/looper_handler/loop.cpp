...
class AReplyToken {
public:
    AReplyToken(const sp<ALooper> &looper) : mLooper(looper), mReplied(false) {}
...
private:
    ...
    sp<ALooper> getLooper() const { return mLooper.lock(); }
...
AHandler::AHandler() : mID(INVALID_HANDLER_ID) {}
...
void AHandler::deliverMessage(const sp<AMessage> &msg) {
    printf("%s[%d] msg.what:%d\n", __FUNCTION__, __LINE__, msg->what());
...
public:
    ...
    handler_id registerHandler(const sp<ALooper> looper, const sp<AHandler> &handler) {
...
ALooper::ALooper() : mRun(false), mRunningLocally(false) {}
...
/**
 * @breif
 * 将一个handler注册到该looper上执行。一个handler只能注册一次，如果要注册到其他looper上，需要先unregister
    ...
    if (runOnCallingThread) {
        {
            ...
            if (mRun) return INVALID_OPERATION;
...
void ALooper::post(const sp<AMessage> &msg, int64_t delayUs) {
    ...
    mEventQueue.insert(
            it, event); //在it前插入event，所以如果有时间相同的事件，那么就插入到时间相同的事件后
}
...
status_t ALooper::postReply(const sp<AReplyToken> &replyToken, const sp<AMessage> &reply) {
    ...
    if (err == OK) {
        mRepliesCondition.notify_all(); // mRepliesCondition不区分reply
                                        // token，也就是唤醒所有在等待的reply token
    }
    return err;
}
...
AMessage::AMessage() : mWhat(0), mTarget(INVALID_HANDLER_ID), mNumItems(0) {}

AMessage::AMessage(uint32_t what, const sp<AHandler> &handler) : mWhat(what), mNumItems(0) {
    setTarget(handler);
}
