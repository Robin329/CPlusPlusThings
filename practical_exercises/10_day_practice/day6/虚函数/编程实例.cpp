...
public:
    ...
    float getSalary() { return workerMoney; }
    void print() {
        cout << "销售员：" << getName() << "\t\t 编号: " << getID() << "\t\t 总工资: " << getSalary() << endl;
    }
...
public:
    HourWorker(string name, string id, int h) : Employee(name, id) { TotalMoney = h * hourMoney; }
