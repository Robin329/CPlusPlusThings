#include <stdio.h>

#include <bitset>
#include <iostream>

#include "base.h"

using namespace std;

int change(int num) {
    if (num < 0) {
        num = ~(num - 1);
    } else if (num > 0) {
        num = ~num + 1;
    }
    return num;
}
template <typename T>
void BinaryBitset(T n) {
    std::cout << bitset<sizeof(T) * 8>(n) << "b" << std::endl;
}

// method 1
unsigned int reverse(register unsigned int x) {
    BinaryBitset<unsigned int>(x);
    x = (((x & 0xaaaaaaaa) >> 1) | ((x & 0x55555555) << 1));
    x = (((x & 0xcccccccc) >> 2) | ((x & 0x33333333) << 2));
    x = (((x & 0xf0f0f0f0) >> 4) | ((x & 0x0f0f0f0f) << 4));
    x = (((x & 0xff00ff00) >> 8) | ((x & 0x00ff00ff) << 8));
    return ((x >> 16) | (x << 16));
}

// method 2
static const unsigned char BitReverseTable256[] =
        {};

// Option 1:
#define RECERSED(v)                                                                      \
    (BitReverseTable256[v & 0xff] << 24) | (BitReverseTable256[(v >> 8) & 0xff] << 16) | \
            (BitReverseTable256[(v >> 16) & 0xff] << 8) | (BitReverseTable256[(v >> 24) & 0xff])

int main() {
    printf("%d\n", change(5));
    printf("%d\n", change(-5));
    printf("%d\n", change(0));
    char c[8];
    c[1] = 1;
    c[2] = 2;
    uint8_t *cc = new uint8_t[8];
    printf("%d %d\n", c[1], c[2]);
    cc[1] = 1;
    cc[2] = 2;
    printf("%d %d\n", cc[1], cc[2]);
    delete[] cc;
    cout << sizeof(unsigned int) << endl;

    printf("after reverse:\n");
    BinaryBitset<unsigned int>(reverse(0xabcde));
    printf("-------------\n");
    BinaryBitset<unsigned int>(0xabcde);
    BinaryBitset<unsigned int>(RECERSED(0xabcde));

    return 0;
}