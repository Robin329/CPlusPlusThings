...
void printArray10(ostream& os, int* a, int n = 0) {
    if (!n) n = 10;
    if (!a) return;
    for (int i = 0; i < n; ++i) os << "Int " << i << ": " << a[i] << "	Addr: " << &a[i] << endl;
}
...
vector<int*> allocateVector(int n) {
    vector<int*> temp;
    int counter = 100;
    for (int i = 0; i < n; ++i) temp.push_back(new int{counter++});
