...
public:
    ...
    //
    // To generate the next ugly number, we can use the existed ugly numbers to
    // multipy 2,3,5, and get the minimal one.
    //
    //     Let's start from the first ugly number - [1]
    //
    //        [1]  next = min( 1*2=2, 1*3=3, 1*5=5) = 2
    //
    //     Now we have [1,2], we can see, only the second one need be multipied by
    //     2 but both 3 and 5 still need be multipied by first one. So:
    //
    //        [1,2]  next = min(2*2=4, 1*3=3, 1*5=5) = 3
    //
    //     Now we have [1,2,3], we can see the second one need be mulityped by 2
    //     and 3, but the 5 still needs be multipied by first one. So:
    //
