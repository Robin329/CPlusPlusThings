...
/*****************************************************************************************************
 *
 * You are given an array nums​​​ and an integer k​​​​​. The XOR
 *of a segment [left, right] where left
 * <= right is the XOR of all the elements with indices between left and right,
...
/*
    ...
    So we just need to obtain the first k length subarray and the rest would be
   determined by it.
...
public:
    int minChanges(vector<int> &nums, int k) {
        ...
        // dp[i][v] means minimum number of changes in first i elements such that
        // the xor value is `v`
