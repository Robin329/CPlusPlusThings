...
having bitwise XOR less than K. Follow the steps below to solve the problem:
  ...
  - Create a Trie store the binary representation of each element of the given
array.

  - Traverse the given array, and count the number of elements present in the
Trie whose bitwise XOR with the current element is less than K and insert the
binary representation of the current element.
...
Notes: for the 1st bit of X, it could be `0` or `1`.
   ...
   - `1`: after XOR E, the 1st bit is `1`, so, we have to check the rest bits ->
Step Two
