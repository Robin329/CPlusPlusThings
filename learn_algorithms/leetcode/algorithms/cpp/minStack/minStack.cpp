...
public:
    Stack() : _capacity(1), _top(-1) { _stack = (T *)malloc(_capacity * sizeof(T)); }
...
public:
    void push(int x) {
        if (stack.empty()) {
            ...
            if (min >= x) {
                min = x;
                // if current minial number already pushed, then just add the reference
                // coount.
...
int main() {
    ...
    cout << "--- expected output [2147483647 2147483646 2147483646 2147483647 "
