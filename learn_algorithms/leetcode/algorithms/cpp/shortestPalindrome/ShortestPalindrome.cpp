 ...
 *
 *   1) We need construct a mirror string base on original string. and use a
 * uniq char for seperator.
 *
 *       considering the original string is "abab", then the mirror string is
 * "abab#baba" (# is seperator)
 *
 ...
 *
 *      For the entire pattern (mirror string), there has max 3 length of
 * sub-string is both prefix and suffix.
 *
 *      This sub-string is "aba".
 *
 *   4) The result of 3) is really useful to solve this shorest  palindrome
 * problem.
 *
 *      Because "aba" is the pattern which in original string and its reversed
 * version.
 *
 ...
 *
 *      So, if a sub-string "aba" is existed in both original string and its
 * reversed version, which means, this is the longest prefix palindrome.
 *
 *    5) Then, we can take rest suffix, and reverse it and put it in front of
 * original string.
 *
 */
