 ...
 *
 *  In other words, if we maintain the prefix sums sorted, and then are able to
 * find out
 *  - how many of the sums are less than 'lower', say num1,
 *  - how many of the sums are less than 'upper + 1', say num2,
 *  Then 'num2 - num1' is the number of sums that lie within the range of
 * [lower, upper].
 *
 */
...
private:
    ...
    int LessThan(Node *root, long long sum, int val, int res) {
        ...
        if (sum - root->val < val) {
            // if (sum[j, i] < val), which means all of the right branch must be less
            // than `val` so we add the amounts of sums in right branch, and keep
            // going the left branch.
            res += (root->cnt - (root->left ? root->left->cnt : 0));
            return LessThan(root->left, sum, val, res);
        } else if (sum - root->val > val) {
            // if (sum[j, i] > val), which means all of left brach must be greater
            // than `val` so we just keep going the right branch.
