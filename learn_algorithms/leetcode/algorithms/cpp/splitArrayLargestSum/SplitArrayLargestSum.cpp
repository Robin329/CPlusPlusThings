...
public:
    ...
    //   3) Then, we use Binary Search to find the resullt between the `min` and
    //   the `max`
    ...
    //   1) if the number of `sub array` >  m, whcih means the actual result is
    //   greater than `sum` 2) if the number of `sub array` <= m, whcih means we
    //   can have `smaller sum`
    //
