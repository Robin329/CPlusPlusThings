 ...
 *seats numbered from 1 to n. All seats are initially available. int reserve()
 ...
 *the lowest numbered seat, which is 1. seatManager.reserve();    // The
 ...
 *lowest of them, which is 4. seatManager.reserve();    // The only available
 *seat is seat 5, so return 5. seatManager.unreserve(5); // Unreserve seat 5, so
 *now the available seats are [5].
 *
 * Constraints:
 *
 * 	1 <= n <= 10^5
 ...
 *one unreserved seat. For each call to unreserve, it is guaranteed that
 *seatNumber will be reserved. At most 10^5 calls in total will be made to
...
public:
    ...
    void unreserve(int seatNumber) { idle.insert(seatNumber); }
};
