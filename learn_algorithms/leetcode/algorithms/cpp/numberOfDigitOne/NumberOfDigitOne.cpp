 ...
 *     c == 1,  the # of `1` appears in units is  "ab * 1 + 0 + 1" (0 means the
 * number after `c`) c > 1 ,  the # of `1` appears in units is  "(ab+1)*1"
 *
 *  3) at second, we seprate it to `a` & `bc`, then,
 *     b == 0,  the # of `1` appears in tens is  "a * 10"
 *     b == 1,  the # of `1` appears in tens is  "a * 10 + c + 1" (`c` means the
 * number after `b`) b > 1 ,  the # of `1` appears in tens is  "(a+1)*10"
 *
 *  4) at thrid, we seprate it to `` & `abc`, then,
 *     a == 0,  the # of `1` appears in tens is  "0 * 100" ( 0 menas the number
 * before `a`) a == 1,  the # of `1` appears in tens is  "0 * 100 + bc + 1"
 * (`bc` means the number after `a`) a > 1 ,  the # of `1` appears in tens is
 * "(0+1)*100"
 *
 *
 *  For some examples:
