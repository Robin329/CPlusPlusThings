...
public:
    ...
    //   1) if we met a non-null node, then this node will generate two child
    //   node. 2) if we met a null node, then this node will generate zero child
    //   node.
    //
    // so the idea is,
    //   1) we can have a counter to calculate how many node we are going to
    //   expect 2) once we have the expected node, then we can decrease the
    //   counter. 3) finally, we will check the counter is zero or not.
    //
    ...
    //      1.2) but the current node also meet the expection of parent node , so,
    //      it need remove 1 in counter.
