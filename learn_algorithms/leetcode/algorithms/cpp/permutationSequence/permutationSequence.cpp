...
string getPermutation(int n, int k) {
    ...
    // Idea: group all permutations according to their first number (so n groups,
    // each of (n-1)! numbers), find the group where the k-th permutation belongs,
    // remove the common first number from the list and append it to the resulting
    // string, and iteratively construct the (((k-1)%(n-1)!)+1)-th permutation
    // with the remaining n-1 numbers
    int group = total;
    stringstream ss;
    while (n > 0) {
        group = group / n;
