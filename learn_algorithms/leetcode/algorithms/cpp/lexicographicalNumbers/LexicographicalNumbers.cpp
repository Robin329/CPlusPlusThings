 ...
 ***************************************************************************************/
class Solution {
    // Solution 1: convert the int to string for sort, Time complexity is high
    // (Time Limited Error)
public:
...
public:
    vector<int> lexicalOrder03(int n) {
        ...
        while (result.size() < n) {
            ...
            // Step Two
            // ---------
            // After find the number which multiply 10 greater than `n`, then go back
            // the previous one, and keep adding 1 until it carry on to next number
            // for example:
            //    curr = 100, then we need evalute:  11,12,13,14,15,16,17,18,19, but
            //    stop at 20 curr = 230, then we need evaluate: 24,25,26,27,28,29, but
            //    stop at 30.
            curr = curr / 10 + 1;
            ...
            //
            // Assuming the n is 234, and Step One evaluted 190, and Step Two,
            // evaluted 191,192,...,199 Now, the `curr` is 200, and we need start from
            // 2 instead of 20, that's why need keep dividing 10
