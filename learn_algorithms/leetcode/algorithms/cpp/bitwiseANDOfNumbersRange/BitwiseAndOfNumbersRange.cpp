...
/*
    Idea:
    1) we know when a number add one, some of the right bit changes from 0 to 1
   or  from 1 to 0 2) if a bit is 0, then AND will cause this bit to 0
   eventually. So, we can just simply check how many left bits are same for m
   and n.
    ...
    for example:
        ...
        11 is 1011
        12 is 1100
        Comparing from 9 to 12, we can see the first left bit is same, that's
   result.
*/
