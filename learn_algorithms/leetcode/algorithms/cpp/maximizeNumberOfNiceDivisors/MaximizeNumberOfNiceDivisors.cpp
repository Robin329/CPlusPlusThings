 ...
 *primeFactors. The number of nice divisors of n is maximized. Note that a
 *divisor of n is nice if it is divisible by every prime factor of n. For
 *example, if n = 12, then its prime factors are [2,2,3], then 6 and 12 are nice
 *divisors, while 3 and 4 are not.
 *
 * Return the number of nice divisors of n. Since that number can be too large,
 *return it modulo 10^9
 * + 7.
 *
 ...
 *[10,20,40,50,100,200]. There is not other value of n that has at most 5 prime
 *factors and more nice divisors.
 *
 * Example 2:
 *
...
/*
    ...
    1) [2,3,5,7,11] - all of factors are different, then we only can have 1 nice
   divisor 2) [2,2,3,5,7] - we can have 2*1*1*1 = 2 nice divisors: 2*3*5*7 and
   2*2*3*5*7 3) [2,2,3,3,5] - we can have 2*2*1 = 4 nice divisors: 2*3*5，
   2*2*3*5， 2*3*3*5， 2*2*3*3*5 4) [2,2,3,3,3] - we can have 2*3 = 6 nice
   divisors 5）[2,2,2,2,3] - we can have 4*1 =4 nice divisors: 2*3, 2*2*3,
   2*2*2*3, 2*2*2*2*3 6) [2,2,2,2,2] - we can have 5 nice divisors: 2, 2*2,
