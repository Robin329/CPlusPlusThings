...
/*
 * For the problem - only one number appears once when all other numbers appears
 * exactly twice.
 *
 * We know, we can XOR all of the array elements. Since X^X is zero, and X^0 is
 * X, so all of the duplicated number will zero themselves out, and the only
 * number would be the result.
 *
 * However, this solution cannot be applied directly to finding two numbers that
 * appear once each.
 *
 * Suppose that these numbers that appear once are X and Y, and all other
 * numbers appear twice. If we decide to XOR all the array's elements, the
 * overall result would actually be `X^Y`.
 *
 ...
 *
 * This information is valuable in sense that we know pieces of information that
 * differ. If we pick up any bit that is 1 in X XOR Y, we can use it as a mask
 ...
 *
 *
 * Now that we have the mask with exactly one bit set to 1, we can walk through
