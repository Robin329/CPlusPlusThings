...
public:
    ...
    // DFS algorithm
    void helper(const string &num,
                const int target,       //`num` and `target` never change
                ...
                long long prev,         // the lastest value we used for calculation,
                                        // which used for operation prioirty adjustment
                char preop)             // the latest "+" or "-" operation, which used for
                                        // operation prioirty adjustment
    {
        ...
        for (int i = idx; i < num.size(); i++) {
            ...
            if (solution.size() == 0) {
                ...
                // if we meet multiply operation, we need adjust the calcualtion
                // priority
