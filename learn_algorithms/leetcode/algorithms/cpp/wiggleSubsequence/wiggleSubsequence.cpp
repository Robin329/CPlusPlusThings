 ...
 * 1) O(N)
 *
 * We notice that adding a new number to an existing subsequence means finding
 * one that is smaller or bigger than the previous number, according to the
 * difference between the previous number and the number before that as we
 * always need to alternate between increasing and decreasing subsequences. If
 * we encounter increasing or decreasing sequences of 2 or more consecutive
 * numbers we can treat the entire subsequence as a number, because that way we
 * can always be sure we don't miss any solution, as finding a number smaller
 * than any number of an increasing subsequence is guaranteed to be smaller than
 * the biggest number in the subsequence. Thus, we can only check the difference
...
public:
    int wiggleMaxLength(vector<int> &nums) {
        ...
        if (nums.size()) {
            solution = 1;
            int bigger = 0; // 0 is the starting point to be followed by either an
