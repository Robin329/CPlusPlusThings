...
//
// 1) We traverse all bars from left to right, maintain a stack of bars. Every
// bar is pushed to stack once. 2) A bar is popped from stack when a bar of
// smaller height is seen. 3) When a bar is popped, we calculate the area with
// the popped bar as smallest bar. 4) How do we get left and right indexes of
// the popped bar –
//    the current index tells us the ‘right index’ and index of previous item in
//    stack is the ‘left index’.
//
//
// In other word, the stack only stores the incresing bars, let's see some
// example
//
// Example 1
// ---------
...
//
//          height[stack_pop] *  i - stack[current_top] - 1,   if stack is not
//          empty height[stack_pop] *  i,                            if stack is
//          empty
//
int largestRectangleArea(vector<int> &height) {
    ...
    vector<int> stack;
    // add a flag as a trigger if the end bar is met, and need to check the stack
    // is empty of not .
    height.push_back(0);
    int maxArea = 0;
    for (int i = 0; i < height.size(); i++) {
        ...
        // If this bar is smaller than the top of stack, then keep removing the top
        // of stack while top of the stack is greater. Let the removed bar be
        // height[top]. Calculate area of rectangle with height[top] as smallest
        // bar. For height[top], the ‘left index’ is previous (previous to top) item
