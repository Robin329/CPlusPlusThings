 ...
 *alphabet in s, and both 'A' and 'a' appear. "aAa" is the longest nice
 *substring.
 *
 * Example 2:
 *
...
class Solution {
    inline int getCharIndex(char c) { return (c >= 'A' && c <= 'Z') ? c - 'A' : c - 'a'; }
    inline int getCaseIndex(char c) { return (c >= 'A' && c <= 'Z') ? 1 : 0; }
