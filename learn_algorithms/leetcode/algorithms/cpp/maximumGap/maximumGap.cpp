...
int maximumGap(vector<int> &num) {
    ...
    // Divide the interval [min, max] into n "buckets" of equal size = (max
    // -min)/n
    int bucket_size = (max - min) / num.size() + 1;

    // For each of the remaining n-2 numbers, determin in which bucket it falls .
    // The number num[i] belongs to the kth bucket B[k] if and only if
    // (num[i]-min)/m = k-1

    vector<vector<int>> buckets((max - min) / bucket_size + 1);
    // For each bucket B[k], compute its max & min among the numbers which falls
    // in B[k]. if the bucket is empty, remain it nothing. if the bucket has one
    // number, make this number as both max & min
