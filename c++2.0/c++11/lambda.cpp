...
// mutable决定[]能够被改写  mutable throwSpec
// retType都是选择的,只要有一个存在就得写() retType 返回类型
// ()放参数
...
public:
    ...
    bool operator()(int val) { return val == localVar; }
};
...
int main() {
    ...
    // lambda 匿名函数
    function<int(int x)> ll = [](int x) -> float { return x + 10.0; };
    cout << ll(1) << endl;
    ...
    // （如果没有向构造函数传入cmp，调用的是默认的构造函数，也就是set() :
    // t(Compare()), 这里会报错, 现在不会出问题了!
    ...
    //    vec.erase(remove_if(vec.begin(), vec.end(), LambdaFunctor(x, y)),
