...
public:
    ...
    bool operator<(const MyStringNoMove &rhs) const { return string(this->_data) < string(rhs._data); }
...
struct hash<MyStringNoMove> : public __hash_base<size_t, MyStringNoMove> {
    size_t operator()(const MyStringNoMove &s) const noexcept { return hash<string>()(string(s.get())); }
};
...
int main() {
    long value =
            3000; // vector
                  // 测试结果的MCtor与CCtor结果大于3000000,是因为vector的动态扩容,当容量不够的时候,会动态分配并进行拷贝。
    ...
    //    unordered_multiset<MyStringNoMove>(), value);
    return 0;
}