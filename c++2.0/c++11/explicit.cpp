...
public:
    ...
    Fraction operator+(const Fraction &f) { return Fraction(this->m_numerator + f.m_numerator); }

    void getNumAndDen() { cout << m_numerator << " " << m_denominator << endl; }
...
public:
    ...
    Fraction1 operator+(const Fraction1 &f) { return Fraction1(this->m_numerator + f.m_numerator); }
...
int main() {
    ...
    //    Fraction1 f1(3,5);
    //    Fraction1 f2 = f1+3; // 会将3转换为(3,1)   不会隐式转换    error: no
    //    match for ‘operator+’ (operand types are ‘Fraction1’ and ‘int’)
    //    f2.getNumAndDen();
    P p1 = {77, 5};
    P p2 = {77, 5, 89};
    //    P p3 = {77, 5, 89,99}; //error: converting to ‘P’ from initializer list
    //    would use explicit constructor ‘P::P(int, int, int, int)’
    return 0;
}
