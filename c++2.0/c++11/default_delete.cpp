...
// 对no-copy的改进 将copy ctor与copy assignment 放到private
// ,但是可以对member与friend调用 boost::noncopyable 就用到了PrivateCopy
class PrivateCopy {
private:
...
int main() {
    ...
    z = z1;
    complex<int> a; // 内部不带指针的可以不用写 big three 没有析构 有copy
                    // ctor与copy assignmetn 都是使用=default,没有自己写
