...
// 第二次 把第一次的composited拆分为float与一包,
// float也占4字节,调整到8边界为8字节,那这一步的composited呢?继续往下分析 第三次
// 把第二次的compoisted拆分成sting与一包,
// string占32字节,已经为8倍数,无需调整,而这一步的composited呢?由于到这里一包已经没了,就会调用它的全特化版本,全特环版本为空,sizeof为1,
// 调整到8的倍数为8.
